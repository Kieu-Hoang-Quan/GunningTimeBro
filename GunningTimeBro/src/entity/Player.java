...
public class Player extends Entity {
    ...
    private long invincibleDuration = 400; // ms
    ...
    public void receiveDamageFromEnemy(Rectangle2D.Float attackBox, int damage) {

        if (invincible)
            return;

        if (attackBox.intersects(this.hitbox)) {

            health -= damage;
            if (health < 0) health = 0;

            System.out.println("PLAYER HIT! -" + damage + " HP | Remaining: " + health);
    ...
    private void updateInvincibleTimer() {
        if (invincible) {
            if (System.currentTimeMillis() - lastHitTime >= invincibleDuration) {
                invincible = false;
            }
        }
    }
